#include "tdl-jturner.h"

enum tdljturner_layers {
  _QWERTY,
  _COLEMAK,
  _LYMD,
  _LYFK,
...
const uint16_t PROGMEM tdljturner_keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
 ...
 [_LYMOS] =
  {
    ...
    {________,________,________,________,________,________,________,________,________,KC_BTN1 ,KC_BTN2 ,KC_BTN3 ,KC_BTN4 ,___XX___,___XX___}
  },
  //Systen Keys
  [_LYSYS] =
   {
...
uint16_t* xd75_layer(tdljturner_layers layer) {
  return tdljturner_keymaps[layer];
};

uint16_t* planck_layer(tdljturner_layers layer) {
  uint16_t * layer_array = tdljturner_keymaps[layer];
  return
  {
    {
        layer_array[1][0],
...
bool process_record_tdljturner(uint16_t keycode, keyrecord_t *record) {
